...
| `CLM_HTTP_REPLAY_TRANSPORT` | HTTP-replay transport. `mitmproxy` (the only transport) is the default and the only accepted value; setting `vcrpy` **fails the build** with a migration pointer (the in-process transport was removed in issue #355 — re-record pre-1.10 cassettes with `--http-replay=refresh`). | `mitmproxy` |
