...
| `CLM_HTTP_REPLAY_TRANSPORT` | HTTP-replay transport: `mitmproxy` (out-of-process proxy; matches repeated/concurrent identical requests) or `vcrpy` (in-process, consume-once — the pre-1.10 default). Cassettes are **not** byte-compatible between the two, so a pre-1.10 course must re-record under mitmproxy or pin `vcrpy` during the transition. See `docs/user-guide/http-replay.md` → "Transports". | `mitmproxy` |
