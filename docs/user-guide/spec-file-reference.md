...
| `http-replay` | `"true"` or `"false"` (default `false`). Opts the topic in to HTTP replay: the build routes notebook traffic through a shared out-of-process replay proxy (mitmproxy) and records it to a cassette next to the source (or under a sibling `cassettes/` directory), then replays it on subsequent builds. Covered client libraries: `httpx`, `requests`, `aiohttp` (other stacks cannot strict-replay — see [HTTP Replay](http-replay.md)). Mode is chosen at build time via `--http-replay=<replay\|once\|new-episodes\|refresh\|disabled>` or `CLM_HTTP_REPLAY_MODE`. Requires the `[replay]` extra. |
