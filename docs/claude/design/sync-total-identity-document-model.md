...
| Q4 (delegation half) — **`clm validate`'s split-pair structure delegates to the engine** (2026-08-03). The three checks that paired the halves *positionally* — shared-cell byte parity, tag parity, slide_id set/order parity — collapse into one adapter over `structural_violations`, the oracle the write gate already uses; the containment property pinned in #766 becomes true by construction rather than by that test's vigilance. Measured benefit: positional pairing let one inserted cell offset every later cell and report each offset pair as a tag mismatch — on the 730-deck corpus 25 tag warnings become 20, one deck contributing 6 of which **5 were phantom and 1 real** (#654). The id-order check also stops flagging a one-sided mid-transition id as a divergence (the engine compares only keys common to both halves). **Severity stays validate's policy, not the engine's** — inheriting the gate's severities would escalate a warning into a commit-blocking error in every downstream course repo, so the adapter maps them explicitly and `duplicate-id` is dropped because `_check_slide_ids` already reports it. The companion `for_slide` check is deliberately NOT delegated: it compares *sets*, so it cannot manufacture a positional artefact, and delegating it would need the companion projection — turning a one-sided companion from a warning into an `id-asymmetry` error and replacing a specific message with a generic one. `VerifyViolation` gains an optional `line` so per-cell findings stay clickable. | §8 (verify/gate — the oracle is now shared with validate) | consolidation: one oracle, two severity policies (adversarial-review Q4, delegation half; containment half was #766) |
