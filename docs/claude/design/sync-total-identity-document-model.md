...
- **Id-paired members are sync points (#906, 2026-09-04).** Rule 2's
  ...
  `pool_placement_divergence` (answerless, pool-freezing) instead of
  executing. Id pairs the halves order differently bracket nothing (their
  brackets would cross; the crossing is the `order` row's question).
...
- **A scope with fewer than two two-sided id'd members is
  ...
  (answerless: move the cell or mint an id, then re-report), so the
  divergence is visible but still not mechanically repairable — the
  executor has no "re-home a positional twin across a sibling" move.
