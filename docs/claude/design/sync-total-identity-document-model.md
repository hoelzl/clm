...
| Q5 — **`uniform_drift_side` observation** (2026-08-02). The review-after-translate flow regenerates or hand-reviews one half; every drifted member then frames `translate_edit` pointing at the twin ("the en variant was edited — adapt the twin"), so an agent reading row by row sees N independent requests to rewrite the *other* language when the one answer that resolves all of them is `keep_twin`. The field report cost ~30 pointless decision items to that blindness. The differ now emits one deck-level observation when **every** `translate_edit` drifts on the same side, naming the side, `keep_twin`, and the opposite reading; `verify_translation` rows are counted in the detail so the summary cannot be over-read into a blanket answer. The per-item detail also names `keep_twin`, and the human report prints the observation after the items. **Explicitly not** the review's `drift: source|twin|both` field: `side`/`direction` already ship per item, so that field would add no information — it would add an *inference*, requiring the engine to name one half authoritative, which it cannot observe (the review's own P8 "frame, never guess"). Informational only: it never affects `is_clean` and changes no classification. | §6.4 (report envelope) | additive observation kind — no `WIRE_SCHEMA` change, no new framed action, no model change (adversarial-review Q5) |
