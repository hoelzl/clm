...
- **Rule 2 is only ever reached by shared members and headers** after
  ...
  permanently.
- **Id-paired members are sync points (#906, 2026-09-04).** Rule 2's
  soundness argument silently assumed a group's positional cells form one
  sequence per half — but an id-keyed member present on both halves in the
  same place is *one logical cell at one position*, so a positional twin
  cannot sit before it on one half and after it on the other. Positional
  pairing (the lens) and the pool's per-side alignment to base (the differ)
  therefore hold **inside the spans sync points delimit**, never across
  one. The base records no spans (the key stays `pos:<group>/<kind>/<i>` —
  this is *not* the withdrawn id-delimited-scope rule of
  `sync-slide-hood-is-presentation.md` §13, which re-keyed and churned):
  a slot's span is *learned* from the half whose cell is provably the
  slot's (fingerprint identity) and constrains the other half's alignment;
  spans are read from the current parse, so an id'd cell inserted above an
  edited positional cell on both halves changes nothing. A cross-side
  pairing that still straddles a sync point — a one-sided move across an
  id'd sibling, or a span-less residue guess — frames
  `pool_placement_divergence` (pool-freezing; `de`/`en` adopts that half's
  placement by re-homing the other half's cell, recording-deferred like the
  §5.1 cross-bracket placement) instead of executing. Id pairs the halves
  order differently bracket nothing (their brackets would cross; the
  crossing is the `order` row's question).
...
- **A scope with fewer than two two-sided id'd members is
  ...
  positions per side with no `order` item. Since #906 the *positional*
  cells caught on different sides of it frame `pool_placement_divergence`,
  whose `de`/`en` answer re-homes the other half's cell — so the divergence
  is visible and repairable through the loop, though the lone id'd cell's
  own placement is still only reported through its positional neighbours.
  Minting ids makes order trackable; the structural gate's
  `(slide_id, role)` order-parity check (#719) is the deck-part backstop.
...
| Y8 — **the pool's lone-candidate landed-twin claim requires content affinity** (adversarial review 2026-07-24, remediation Phase 3 item 8). `_align_pool` claimed ANY lone unmatched new cell on a pending-twin slot's missing side as the landed twin; the downstream `pending_divergence` frame's `de`/`en` answer then overwrote a genuinely new, unrelated cell verbatim — framed, so misleading rather than silent. The claim now requires body similarity to the recorded cell (the budgeted `_BodySimilarity` oracle the #630 split scan uses). The no-affinity candidate is a genuine add, not the twin — but it cannot simply keep its add row: the positional pairing marries it to the slot's member (the twin's verbatim copy refuses an occupied target). The slot therefore frames `ambiguous_alignment` with the honest reading (not the twin — mint a `slide_id` or remove the cell, re-report); the candidate's news row is suppressed for the pass when it would render on a marked slot's handle (pool ordinals alias — an answerless frame beside an answerable cold row on one handle defers the cold row's recording forever, the #654 placement-suppression precedent) or, while a group rename is in flight, when it renders that handle under the new anchor; the frame names whose handle it shares, and it frames separately otherwise. The landed-twin claim — byte-identical OR body-similar — binds only at the slot's OWN rendered handle (round 2, Important; round 3, Critical: a cross-position claim's record/divergence resolution writes against a cell the fresh-snapshot rerecord pairs with a DIFFERENT slot — the pool livelocked, the slot's ledger entry silently dropped, the record reporting success forever; the byte-identical variant livelocked exactly like the affine one); a matching cell at another position frames `misplaced` (move it, mint a `slide_id`, or remove it) — both shapes pinned by `_Deck` convergence tests; under a group rename an at-position landing reads as cross-position to the raw key compare, so the frame names the rename instead (no action needed; the candidate's own row is held back for the pass — emitted, it would report a false `recorded` verdict and be erased by the unresolved-pool drop, since the apply-side deferral guard is key-based and the row renders under the new token — so the slot entry survives and the claim binds once the rename records; decision-free convergence pinned by a `_Deck` test). A slot's no-affinity mark also loses to a LATER slot's claim of the same lone candidate (claim-loop order): the earlier slot reframes as claimed-elsewhere (resolve the claiming slot's frame first) rather than contradicting it — with position-bound claims this is defensive (a marked candidate renders at most one slot's handle), kept so a stale mark can never frame a false row-fate note. The documented reconciliation converges mechanically on the next pass. | §6.2 (`conflict` row), §8 (report surface) | defect fix: never advertise an answer whose execution destroys unreviewed content; P8 — frame, never guess |
| #826 — **shifted-pool trust suspension: mechanical pool rows require an unshifted pool** (2026-08-21). The lens pairs id-less cells by cursor with no content check (§3.3's steady-state soundness argument assumes byte-parity *and stable pool membership*), so a mid-pool class transition — a fork/unify absorb claiming a one-sided cell, or a pos→id migration consuming a base entry — re-pairs every later sibling with the wrong twin, and the pool's mechanical rows then executed against that guess: the filed shape overwrote a sibling's tags (`mirror_tags`), the probe variants destroyed whole bodies (`propagate_shared_edit`, always claiming a victim at the pool tail), the unify route corrupted the EN side, and the pass-2 re-record *duplicated* the mis-married cell (`copy_new_shared`) — never converging. The differ now tracks the pools whose per-side accounting it shifted this pass (each absorb site and the migration, including a **body-match with the #644 deficit gate closed** — a fork whose side also gained a new cell backfills the count, so the suspicion must mark even where the migration correctly refuses) and, in ledger mode, suspends the pool's trust: every slot not provably at base on BOTH sides — per-side fingerprint identity is the one cross-side-guess-free evidence — frames one answerless `pool_pairing_shifted` row (resolution `manual`, detail naming the causing transition) instead of executing, banking, or framing misleading evidence; pool order rows and non-neutral cold news frames are suspended the same way (`record_neutral` keeps executing — byte identity of a two-sided member's own cells is proof; a byte-identical *landed* twin still defers one pass, since the "landed" slot state is not same/same — review F5 pinned that the suspension covers it). The action joins apply's `_POOL_FREEZING_ACTIONS` (the #630 dedicated-action rule) — and because a marked pool can be **silent** (migration and absorb both hit their true targets, every slot same/same, zero suspension rows to key a freeze on) while its fresh snapshot still contains the absorbed estranged twin, the differ's shifted set additionally rides on `DeckDiff.shifted_pools` and apply unions it into the freeze wholesale (review F1: a landed `pending_divergence` answer in the silent shape re-recorded the pool, destroyed the migrated slot's two-sided baseline, and mechanically duplicated the cell on the next pass — pre-existing, now closed); a frozen-pool landed row's ItemResult says "recording deferred", never "recorded" (F6). The gate-closed cause member itself frames `stamp_vs_new` instead of taking the mechanical `copy_new_shared` its masked deficit scan would grant (F3), and `item_answers` drops `mark_twin` on a ONE-SIDED `fork_pending_twin` (no twin cell exists to mark; the executor refused it — F4, the M6 no-dead-answers rule; the absorbed candidate is deliberately not offered, it can be the wrong cell). Snapshot mode (`--since`, the forensic view) deliberately keeps the raw reading — nothing executes there. Residue, tracked on the issue: the transition frame itself may still absorb the *wrong* twin (framed, cells visible — the Y8 "misleading rather than silent" class); rescuing the true twin from a mispaired two-sided member needs (id, side)-keyed absorption and stays follow-up work. | §3.3 (the positional-soundness argument's membership condition made explicit), §6.2 (`mechanical` row's trust boundary), §8 (apply's freeze set) | defect fix: the Y5 doctrine at the second pairing authority — mechanical only on proof, frame, never guess; one new answerless framed kind via P8(c) |
| #885 — **one order authority per pass** (2026-08-22). The field failure: on a pair whose order was visibly contested (a framed `order_decision`), apply co-executed the user's scope answer ("adopt DE's order") WITH mechanical member-keyed `mirror_order` rows derived from the contested pre-answer bracketing — two contradictory order authorities in one pass — and the mirrors moved a slide-start cell across its own body cells on the side the answer designated to preserve. The mirrors' base-backed proof was *laundered*: gate-refused passes keep file writes while withholding the ledger ("landed mutations stay"), so a later diff reads the engine's own ungated intermediate as an authoritative author edit. Fix, per-item at apply (the #824 keep-defer shape — P7-clean, no differ change, no P6 post-pass): while any `order_decision` is framed in the pass — answered or not — every mechanical `mirror_order` defers with a reason naming the framed handle, and re-derives from the settled order next pass; an uncontested pass is untouched. Symptom 1 closed the same way: the #720 anchor-adjacency refusal is now typed (`AnchorAdjacencyError`), and while the same pass frames an order question the refusal reports as a **deferral naming that handle** instead of a bare rejection the differ re-frames identically forever (the answer is consumed; the reason says to re-answer after the order settles). Envelope honesty: the apply payload gains a top-level `left_undone` list (rejected/deferred/failed rows — `pending` is the loop's normal state and stays out), so `wrote: true` + exit 1 is never the only signal that answered work did not land. Pre-merge adversarial review added two closures: (C1) `_mirror_group_order` re-emitted each cluster in target-stream encounter order, so the order ANSWER itself re-manufactured a displaced anchor on the adopted-into side — one pass later the re-derived, now-uncontested member mirrors copied that corruption onto the preserved side and the diff went clean; clusters now emit **anchor-first** (anchors bracket their group, so anchor-first IS base placement; a subslide is its own group, so a cluster holds at most its own anchor), and the flagship test walks the loop to convergence with the preserved side byte-identical throughout. (I1) a deferred POOL-handle mirror froze nothing (`_pool_scope` skips `pool.` markers; the row carries no member), so a landed same-pool sibling re-recorded the cursor-married snapshot — deferred pool scopes now join the recording freeze. Residue: how the field EN reached its corrupted intermediate is unpinned (the insert path provably cannot produce it; fuzzing from clean states never did) — with C1 closed the engine no longer manufactures such intermediates itself, and the sequencing rule removes the compounding for externally-arising ones. | §8 (apply sequencing + envelope), §6.2 `order` row's trust boundary | defect fix: the conflict_tags suppression doctrine mapped to the order family — sequencing, not demotion, because the mirror's own evidence may be genuinely trust-backed for its scope; deferral is the P7 primitive for "not this pass" |
| #892 — **`--stamp-ids` falls back to the doc-lens pairing; one-sided cells become stampable** (2026-08-22). §3.4 assumed "run `normalize --stamp-ids`" always works, and the report's normalize-first refusal leans on that assumption — but the stamp gate was a *text-level* unify round-trip, and a one-sided new cell is unrepresentable in the unified form, so the very decks that need stamping are the ones the gate refused: report said "normalize first", stamp-ids said "fix the alignment first", and the only escape was hand-writing `slide_id=`s into a split half (the #892 field report: 3 report rounds + 7 hand-written ids). The gate's job (no divergent twin slugs) is a *pairing* question, and P4 says pairing happens on the parsed model — so when unify fails, stamping now runs from `parse_bundle`, the engine's ONE pairing authority: a refused parse whose reasons are all `NORMALIZE_FIXABLE` hands the stamper its paired document via `ParseOutcome.provisional_deck` (a strict never-diff contract — the refusal stands). What stamps, from the lens's own twin/no-twin facts: id-less localized/narrative members pair-atomically (both sides one EN-authority slug — member-level atomicity, strictly stronger than the file-level gate); **one-sided members solo** — an id-less anchor, or a shared cell inside a two-sided group (the ordinal-aliasing shape whose `verify_cold` was an answerless dead end; its detail now names the command) — no twin exists, so no slug can diverge, which is why §3.4's "shared cells are not stamped" gains exactly this clause (the no-churn rationale is untouched: members of a ONE-sided group keep positional identity — their twin pool is empty, no aliasing); and **adjacent one-sided id-less anchor twins pair by cursor** with their groups' localized members, per (kind, narrative-hood) bucket — the lens's own pool classes, never blind position across kinds (review I4) — because solo-stamping twins would mint exactly the divergent slugs the gate exists to prevent. The synthetic `~idless@<line>` tokens are line-number-derived and **can merge** at equal line numbers (review I3 refuted the "never merge" premise): a merged two-sided id-less group is its own twin — stamped directly, once, members lens-paired — and only one-sided id-less groups enter the cursor runs. A non-id-less refusal (`duplicate_id`), or a refusal-backed parse with nothing stamped and nothing refused, keeps the loud pair-level refusal; a CLEAN parse returns a possibly-empty result instead (review I1: the second run over a lens-stamped deck stays non-unifiable forever while being fully normalized — refusing there made `normalize` exit 2 on a deck sync accepts, and starved companion stamping). An unusable heading records a per-cell refusal, never a silent skip (review I5). The `verify_cold` remedy names `normalize --stamp-ids` only where the stamper will act — a one-sided member of a ONE-sided group keeps the hand-repair wording (review I2: the unconditional remedy was a new report↔normalize circle of exactly the #892 species). **Residue, deliberate**: the cursor marriage stamps what the lens can see — a twin the lens cannot pair (an id-less copy of an already-id'd slide; slide-hood divergence on an id-less pair; different slides added into one gap) mints a wrong-but-FRAMED identity, visible in the stamp report, framed by the next sync report, repaired by `rename-id`; the old behavior was refusing everything. With #895 (all refusal classes in one report), the loop closes: report → `normalize --stamp-ids` → report → decisions, no hand-edits. | §3.4 (the precondition's repair path made total; the shared-cell no-stamp rule gains its one-sided clause), §3.3 (the lens as sole pairing authority now serves normalize too) | gap fix in the §3.4 assumption — the fix moves stamping ONTO the existing pairing authority instead of teaching the engine cold-pairing heuristics (the B1 "mint_id rows in report" alternative was rejected: it re-enters normalization into the loop §3.4 deliberately keeps out, and needs apply-on-refused-deck + freshness-token carve-outs) |
| #906 — **id-paired members are sync points; the pool aligns per span** (2026-09-04). The field failure: a setup pool with an id'd cell mid-pool; one EN pass deleted two positional cells and edited the remaining three. Both positional authorities guessed across the id'd cell — the lens's cursor married the DE cell in front of it to the EN cell behind it, and the differ's per-side `SequenceMatcher` residue (a count-mismatched *replace*, a positional guess by its own comment) paired the same slots — so the mechanical `propagate_shared_edit` wrote EN's `create_rag_system` into DE's deleted-`chat_prompt` slot **before** the `docs_content` it depends on, `mirror_remove` deleted DE's own copy behind the id'd cell, the result passed `sync verify` and banked, and only the strict-replay build caught the `NameError`. Root cause: §3.3's soundness argument assumed one positional sequence per half, but an id-paired member is one cell at one position — a sync point no positional pair may cross. Fix, at both authorities from the same rule: the lens pairs id-less cells inside the spans a region's uncrossed id pairs delimit (`_sync_spans`); the differ builds a per-half span index (nearest preceding two-sided id member in the same bracket/owner) and aligns each pool **per span** — a slot's span is learned from a half whose cell is fingerprint-identical to the slot's (stage 1 unique matches; stage 3 equal blocks of the better-witness half, `de` on a tie) and constrains the other half's positional residue (stage 2), so the base needs no span field and an id'd cell inserted above an edit on both halves churns nothing (pinned). A slot whose halves still sit under different sync points — a one-sided move across an id'd sibling (silent before: the §9 order-untrackable residue), or a span-less guess — frames the new `pool_placement_divergence` (outcome `order`, apply's `_POOL_FREEZING_ACTIONS`, also an order authority for the #885 one-authority rule) instead of executing; its `de`/`en` answer re-homes the other half's cell next to the chosen half's mirrored predecessor through a holder-rule-aware sibling of `_mirror_member_move`, recording-deferred like the §5.1 cross-bracket placement (the pairing was a guess, so the slot re-derives at the settled placement instead of banking it). Deliberately **not** the withdrawn id-delimited-scope rule (`sync-slide-hood-is-presentation.md` §13): keys, ledger schema and the change-point count are untouched — spans are cross-side *evidence*, not identity. Residue: the crossed-id-pairs shape frames both the order row and the placement row. | §3.3 (sync-point rule), §9 (the order-untrackable residue narrowed), §8 (one new framed kind with the `order_decision` answer shape) | defect fix: the Y5/#826 doctrine at the pool's third guess — mechanical only inside a span; P8(c) for the frame |
| #900 — **a fork in flight pairs with its true twin at the lens** (2026-09-04). #826's residue: the transition frame absorbed the wrong twin because the lens's `_pair_class` put the marked half (`lang=` + id) into the *localized* class, the shared pool's cursor shifted, and the true twin was married into a two-sided sibling member the one-sided absorb helpers could not reach. The issue proposed (id, side)-keyed absorption in the differ; the fix goes one authority lower, where the mispairing is made: langness is member state (P2), so an id'd lang-tagged cell left unpaired by its own class pool is **offered to the lang-less pool of its kind** as a slot occupant, and `pair_positionally`'s existing #443 adoption (byte-equal body at the cursor, the #716 residue rule) marries it to its untransitioned twin — the same proof the stamp adoption already relies on, and the Y5 gate's `pre_fork_fingerprint` already anticipated this pairing. Consequences, all in existing rows: the fork frames two-sided with the true twin (`mark_twin` advertised, and the executor now writes lang **and** the fork's id when the twin is id-less — §7.3 mints the id at fork time, so the differ folds the ledger-known pending stamp into the fork frame instead of co-emitting `stamp_twin_id`: one framed row per member); sibling slots read same/same and are never suspended; `_match_key_migration` takes an exact content match over the body-only fork channel (identical bodies matched the first sibling's entry). The V4 shape (fork + edited twin) has no byte proof, stays one-sided and keeps the #826 suspension, as the issue's design note required; `absorbed_pos` stays member-keyed. | §3.3 (the lens's pairing classes: langness is not a class boundary for an id'd cell), §7.3 (fork mints the id through the frame) | defect fix at the pairing authority (P2/P4) instead of a second rescue mechanism in the differ (P8: no new mechanism) |
| Y9 — **record/confirm hardening** (adversarial review 2026-07-24, remediation Phase 3 item 9, the LOW bundle; its third note — apply writes before the structural verify — stays its own change per the handover's landmines). (a) `record` never consulted the diff: the structural gate cannot see the localized-pair frames (`verify_cold`, `verify_translation` are structurally clean), so a warm deck with unanswered questions was wholesale-blessed silently. `record` now diffs against the existing ledger and warns (stderr + a `pending_framed` JSON list) when framed items are pending — it still records (the trust verb's point), but the blessing is no longer silent; mechanical residue and the cold bootstrap do not warn. (b) `verify_cold`'s `confirm` banked a byte-diverged SHARED member as verified (observed on separated voiceover companions, whose structural record gate the confirm path never consulted): the new `_reject_divergent_shared` guard sits beside `_reject_divergent_tags` in the confirm route and rejects a shared member whose sides' fingerprints differ — localized pairs may diverge (that is what translation is) and `j2` header members are exempt (the `header_de`/`header_en` macros legitimately differ per half; the structural gate excludes them for the same reason). The tested escape: answer with a `body` naming the stale `side` when the bodies differ and at least one side provides non-whitespace content, then confirm; when no usable body answer exists (tag order, owner, kind, trailing separator counts, or both bodies empty/whitespace-only), align by hand — the report advertises no answer for those shapes (`resolution: manual`, repair named in the item's detail per the M6 doctrine). The report never advertises an answer the guard rejects (the standing no-dead-answers rule): `item_answers` drops `confirm` for any byte-diverged shared pair — `verify_translation` is reachable on a shared member via the stamp + tag-order route — and drops `body` unless `body_reconciliation_available` proves a validated source can change the target. | §8 (record/apply), §6.2 (guard semantics) | hardening: warn on wholesale blessing; never bank a divergent shared pair as verified |
| #992 — **the apply write gate is member-scoped** (2026-09-25). The field failure (#787, 2026-09-24): thirteen decisions in one document, twelve bodies applied, one malformed `verify_translation` body rejected — and the next report re-framed every applied body as `verify_translation`, costing a full `confirm` round. The rejection was a coincidence: the pass also carried a hand-removed EN slide still framed `remove_localized_side`, whose `id-asymmetry` (and, once the removal landed, the orphaned companion cell's projection refusal) failed the post-write structural verify, and `apply` gated the WHOLE ledger save on it — every landed member, including the twelve on untouched slides, was unrecorded. The engine-side recording was already member-scoped (`unresolved_keys`, `_frozen_pools`); only the CLI's save was all-or-nothing. The gate now runs inside `apply_deck` (`verify_gate`, after the write, before recording) and is scoped like the harvest per-slide gate: a violation naming a `slide_id` withholds the recording of the group holding that id (or owned by it, for companion cells) — the withheld rows keep their file mutation and old baseline, carry a reason suffix, and are listed in `ApplyOutcome.verify_withheld` — while a violation naming no slide (`unify`, `order-parity`, a mixed / cross-language layout) still withholds everything. `gate_projected_pair` attributes an orphaned-narration refusal per missing owner (bare ids in `ProjectedPair.unmatched`; an orphan with no `for_slide` keeps the refusal deck-wide, and a scoped call keeps its caller's `slide_id`) so the field's second stage is scoped too. Fail-safe (adversarial review): ids are compared bare (the verify names them with the `!` preserve marker), and a violation naming an id neither parse can place is treated as deck-wide — never a silent no-op gate. `ledger_recorded` now means "saved with at least one record" and can be true beside `verify_violations`. `record` (wholesale by construction) keeps the deck-wide gate. | §8 (apply/record), the write gate's scope | defect fix: the gate's scope matched `record`'s wholesale semantics, not `apply`'s per-member recording; mechanical rows inside a faulted group are withheld conservatively and re-derive next pass |
| #993 — **the report's `de_body`/`en_body` are the decision body for every member, header rows included** (2026-09-25). The field failure (#787, 2026-09-24): a retitled deck framed `id:title` as `verify_translation` with empty bodies — the macro line showed only in the `de`/`en` excerpts — and two agents independently guessed that the answer is the full `# {{ header_de("…") }}` line. Cause: the excerpt projection used `SideCell.body` (the cell below its delimiter), which is empty by construction for a single-line j2 macro cell, while #609's executor replaces exactly that line. Fix at the cell model: `SideCell.is_macro` (the predicate #609 kept private in the executor) and `SideCell.decision_body` (`header` for a macro cell, `body` otherwise); the report projects `decision_body`, the executor tests `is_macro`, so the two authorities share one rule and the M10 property — report output is valid decision input — now holds for header rows. Fingerprints keep using `body`; no ledger change. The report's second half (an applied header body that stayed framed) was the #992 deck-wide gate, not a header defect — the harness banks a header body in one apply, now pinned. | §6.4 (item payload: the body projection), the #609 row | defect fix: one predicate for "this cell's content is its line" at both ends of the loop (P8: no new mechanism) |
| #991 — **a deck stem rename migrates the ledger section** (2026-09-25). §12 decision 4 keeps deck identity path-derived (`deck_key_for` = the language-free stem), so a renamed deck cold-starts — fail-safe, and the field procedure was a hand edit of the ledger's deck key, the riskiest step of a four-place rename. `clm slides rename` now owns it: halves + companions move together, and `doc_ledger.rename_deck_key` re-keys the section as a pure rename — the members, order scopes and preamble fingerprints are keyed by `slide_id` / position, never by the stem, so nothing is re-fingerprinted and the deck stays warm (`rename-id`'s key migration, one level up). The re-key is a direct file rewrite, not a `save`: the M8 section merge cannot express dropping a section, and a rename must drop the old key — acceptable for a deliberate one-off operation that is not a sweep verb. Refuses when the new key already has a section (never clobber another deck's trust). Deck identity stays path-derived; only the command that moves the path also moves the key. | §12 decision 4 (deck identity), §5 (the ledger as the only trust store) | affordance: the trust store follows a deliberate rename instead of cold-starting; no new mechanism in the differ (P8) |
