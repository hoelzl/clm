...
# (tests import PlantUmlWorker in-process; see _configure_logging).
...
def main():
    ...
    if API_URL:
        ...
        if jobs_db_path is None:
            raise SystemExit(missing_jobs_db_error("plantuml"))
        ...
        # This handles both pre-registration (CLM_WORKER_ID set) and legacy registration
        worker_id = Worker.get_or_register_worker(
            db_path=jobs_db_path, api_url=None, worker_type="plantuml"
        )

        # Create and run worker
        worker = PlantUmlWorker(worker_id, db_path=jobs_db_path)
