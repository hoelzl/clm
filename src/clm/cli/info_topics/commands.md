...
| `--stamp-ids` | (since CLM {version}) The **one-time sync-v3 id normalization** (#520): stamps a `slide_id` onto every id-less **localized** cell (`lang=…`, markdown or code) and gives every voiceover/notes **narrative** its **own unique** content-slug id — re-pointing legacy inherited-owner (and `<deck-stem>-cell-N` placeholder) ids; any other existing id is kept, `!`-preserved ids always win. EN-authority and **pair-atomic**: split decks are stamped through the unified pair so `de_id == en_id`, and a refusal means the deck was left **untouched**. Cells without a directly-adjacent DE/EN twin (including whole block-interleaved runs like `de,de,en,en` — run the `interleaving` operation first) and lone split halves are refused as review items, never half-stamped. A pair the text-level unify walk cannot represent — typically because one half gained **new one-sided cells**, the very shape that needs stamping — is stamped from the **doc-lens pairing** instead (since CLM {version}, #892): id-less localized/narrative twins get one EN-authority slug per pair, one-sided cells (an id-less anchor; a shared cell inside a two-sided group) are stamped solo, and an id-less anchor added to both halves pairs with its twin so the slugs cannot diverge. Only a pair the lens cannot own either (a `duplicate_id`, or nothing to stamp) is refused. Directory discovery is **prefix-agnostic** for split decks (`apis.de.py` is found alongside `slides_x.de.py`). **Voiceover companion files** (`voiceover_*.de/en.py`) are stamped together with their deck pair: each companion cell's `slide_id` becomes its own unique id while `for_slide`/`vo_anchor` (the owner/placement references) stay untouched; the deck pair and its companions share one id namespace, the halves must mirror (same cell count and `for_slide` sequence — else the companion pair is refused untouched), and a companion existing for only one half is refused. Two-sided shared language-neutral cells are never stamped (positional identity is sound for them); a **one-sided** shared cell inside a two-sided group is stamped since CLM {version} — it has no twin to diverge from, and without an id it dead-ends sync's `verify_cold` (#892). **Replaces the regular operations for that run** (combining with `--operations`, `--confirm-pairs`, or `--canonicalize-start-completed` is an error); honors `--dry-run`, `--json`, and the directory scoping options. |
...
never-recorded adoption frames one `verify_translation` row with every other
row for the member suppressed that pass (no `record_fork`, `mirror_tags`, or
order mirror may execute or bank against a pool-order guess), and `confirm`
banks the pairing so the next report stamps.)
...
| `--json` | Emit one JSON array with a document per channel, in spec order (CLM {version}+, issue #967): `channel`, `stream`, `source`, `dest`, `language`, `dry_run`, `partial_manifest`, `skeleton` (`action` `copy`/`frozen`, `file_count`, `present_count`, and the `files` a first sync would freeze), `rows` — one per topic (`kind: "topic"`, `action` `copy` / `refreeze` / `skip-frozen` / `skip-failed`, `topic_id`, `file_count`) and one per skeleton `refresh` (`kind: "skeleton"`, `path`, `label` `evergreen` / `refreeze-skeleton`) — `evergreen_up_to_date`, and `push` (`requested`, plus the commit `message` the push would use). A real run adds `result` (`files_copied`, `copied_topics`, `refrozen_topics`, `skipped_topics`, `failed_topics`, `skeleton_copied`, `refreshed_files`). Notes and warnings go to stderr; exit codes are unchanged. Needs `--dry-run` when combined with `--push` (the push output is not JSON). |
...
clm release sync course.xml --channel jan --dry-run --json # the plan as rows, for an agent
