...
Recordings made before the ledger existed have no entries yet: seed them
from the local state files once with `clm recordings seed-ledger COURSE_ID
--spec-file SPEC` (dry-run first), review the result with `clm recordings
report`, and commit the ledgers. Add `!**/.clm/recordings-ledger.json` to
the course repo's `.gitignore`.
...
- Response `Set-Cookie` headers are dropped, and the values of OAuth-shaped
  ...
  **exactly but case-insensitively** — `completion_tokens` and `total_tokens`
  are untouched, so replayed LLM usage data stays correct, while
  `{"Password": …}` is redacted. A **number, boolean or `null`**
