...
| `html` | If set, skip HTML generation for this topic. Since CLM {version} this also covers the *implicit* Recording-HTML execution a narrowed target set (e.g. `-T shared`, without the `recording` producer) schedules to warm the executed-notebook cache: a topic with no HTML outputs has no HTML consumer to serve, so it is never executed for that reason either — the deck behaves the same under every `-T` selection. |
...
| `partial` | Completed outside workshop ranges, code-along inside them. A workshop range starts at a markdown cell tagged `workshop` **or** a slide-start (`slide`/`subslide`) markdown cell whose `slide_id` starts with `workshop-`, and ends — exclusively — at the next `end-workshop` cell of any type (markdown-only before {version}), the next workshop opener, or end-of-notebook. Intended as a student follow-along artifact: demonstrations remain worked out, workshops stay blank. Without an explicit `end-workshop` tag, the workshop runs to end-of-notebook (legacy behaviour). Use `end-workshop` to mark the end of a workshop in the middle of a deck — on the heading that starts the next non-workshop section, or on the first cell after the exercise. The tagged cell is itself outside the workshop, so tagging the workshop's final code cell excludes that cell from the range (it renders completed instead of blanked; for a `keep`-tagged assertion cell the output is identical either way). `clm validate` (since {version}) warns when a `# Workshop` heading has no workshop scope covering it, so a missing opener doesn't silently render the exercise cells. |
