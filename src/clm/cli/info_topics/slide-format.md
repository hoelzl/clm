...
| `global` | (since {version}, issue #928) **Code cells, C++ code export only.** Emits the cell in the deck header `<deck>.hpp` (namespace scope, shared by the lecture file and every workshop file) instead of inside its section function — the way a workshop file gets to see a lecture definition, and the manual override for a top-level variable the export's reference scan does not promote. Inside a workshop range the cell stays at namespace scope of that workshop's file. See "C++ code export" under `clm build` in `clm info commands`. Orthogonal to `keep`; no effect on any other output. |
