...
def sync(
    ...
    if alignment_override is not None:
        ...
        try:
            alignment = load_alignment_override(alignment_override)
        ...
        for part in parts:
            ...
            if transcript_override is not None:
                ...
                try:
                    transcript = load_transcript_override(transcript_override)
                except OverrideError as exc:
                    raise click.UsageError(str(exc)) from exc
            else:
                console.print(
    ...
    if overwrite:
        ...
        display_notes_summary(notes_map, slide_groups)
        ...
        if use_companion:
            ...
            except MissingSlideIdError as exc:
                raise click.UsageError(str(exc)) from exc
...
# ---------------------------------------------------------------------------
# ``clm voiceover extract`` / ``clm voiceover inline`` — companion-file
# round-trip (formerly voiceover_tools.py).
# ---------------------------------------------------------------------------


@voiceover_group.command("extract")
@click.argument(
    "path",
...
@voiceover_group.command("inline")
@click.argument(
    "path",
    type=click.Path(exists=True, dir_okay=False, path_type=Path),
)
