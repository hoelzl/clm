...
def generate_cmakelists(
    ...
    *,
    ...
    names disambiguated with a numeric suffix. Decks in *excluded* (marked
    ``clm: no-compile`` in the course source) become ``EXCLUDE_FROM_ALL``
    targets: still buildable explicitly, but skipped by "build all" — and
    thereby by the CI compile check.
    """
    lines = [
    ...
    for rel in sorted(deck_rel_paths):
        ...
        if rel in excluded:
            lines.append(f'add_executable({name} EXCLUDE_FROM_ALL "{rel}")')
        else:
