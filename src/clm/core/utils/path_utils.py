"""Course-domain path vocabulary (Phase 8 step A6, #802).

The names, layouts and classification rules of a course tree: output specs
(Lang/Format/Kind), skip/ignore rules for course scanning and output, slide
file-family detection, and prog-lang/extension mapping. Pure path logic — the
filesystem-touching utilities (``find_project_root``, ``atomic_write_*``)
live in :mod:`clm.infrastructure.utils.path_utils`.
"""
