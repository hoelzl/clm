...
**Version**: 1.5.0 | **License**: MIT | **Python**: 3.11, 3.12, 3.13, 3.14
...
- **Slide Authoring Tools**: Topic resolution (`clm resolve-topic`), fuzzy search (`clm search-slides`), spec/slide validation (`clm validate-spec`, `clm validate-slides`), normalization (`clm normalize-slides`), bilingual language view (`clm language-view`), sync suggestions (`clm suggest-sync`), voiceover extraction (`clm extract-voiceover`), stable slide-id assignment (`clm slides assign-ids`), LLM-driven voiceover coverage check (`clm slides coverage`), and structured JSON outlines (`clm outline --format json`)
