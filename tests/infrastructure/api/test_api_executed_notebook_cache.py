...
class TestGet:
    ...
    def test_parses_notebook_json_on_hit(self):
        client = MagicMock()
        nb = _sample_nb()
        client.get_executed_notebook.return_value = serialize_notebook(nb)
        cache = ApiExecutedNotebookCache(client)
    ...
    def test_returns_none_on_malformed_payload(self):
        ...
        client = MagicMock()
        client.get_executed_notebook.return_value = b"not a notebook"
        cache = ApiExecutedNotebookCache(client)
...
class TestStore:
    def test_serializes_notebook_and_forwards_to_client(self):
        client = MagicMock()
        cache = ApiExecutedNotebookCache(client)
        nb = _sample_nb()
        ...
        round_tripped = deserialize_notebook(kwargs["payload"])
