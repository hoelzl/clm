...
def test_is_alternate_solution_for_non_alternate_cell(code_cell, markdown_cell):
...
def test_get_slide_tag(markdown_cell, markdown_slide_cell, markdown_subslide_cell):
...
def test_find_notebook_titles_for_notebook_with_replaced_characters():
    assert find_notebook_titles('{{ header("See: <>?Here!%$", "{/a/b\\\\c/?}") }}') == {
        "en": "(_a_b__c_)",
        "de": "See __Here__",
    }
